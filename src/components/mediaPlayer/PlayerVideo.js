...
const PlayerVideo = ({
  ...
  useEffect(() => {
    ...
    player.on("play", function () {
        ...
        console.log(audioRef.current.currentTime,'hloworld1');
    })
    player.on("pause", function () {
        // player.currentTime = currentTime;
        setTime(audioRef.current.currentTime)
        console.log(audioRef.current.currentTime,'hloworld2');
    })
    ...
    >
      {currentSong && (
        ...
        >
          ...
          <source
            src={currentSong.multipleLanguages.eng.low}
            type="video/mp4"
            size="576"
          />
          <source
            src={currentSong.multipleLanguages.eng.medium}
            type="video/mp4"
            size="720"
          />
          <source
            src={currentSong.multipleLanguages.eng.high}
            type="video/mp4"
            size="1080"
          />
          {/* <!-- Caption files --> */}
          <track
            kind="captions"
            label="English"
            src={require("../../captionPath/captions.vtt")}
            srclang="en"
            default
          />
          <track
            kind="captions"
            label="Français"
            src={require("../../captionPath/frenchCaptions.vtt")}
            srclang="fr"
          />
